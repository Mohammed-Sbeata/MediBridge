...
const signUpSchema = z.object({
  ...
  hospital: z.string().min(1, 'Hospital/Area is required'),
...
export default function LocalSignUp() {
  ...
  const validateReferralCode = async (referralCode: string) => {
    try {
      const response = await fetch(`/api/auth/validate-referral?code=${encodeURIComponent(referralCode)}`);
      const data = await response.json();
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Create your account to connect with external healthcare professionals
          </p>
        </div>
          ...
          <div>
            ...
            {(errors.referralCode || referralError) && (
              <p className="mt-1 text-sm text-red-600">
                {errors.referralCode?.message || referralError}
              </p>
            )}
          </div>
